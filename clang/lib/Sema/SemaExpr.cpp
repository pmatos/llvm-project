...
inline QualType Sema::CheckLogicalOperands(ExprResult &LHS, ExprResult &RHS,
  ...
  if ((LHSTy->isArrayType() &&
       LHSTy->getArrayElementTypeNoTypeQual()->isWebAssemblyReferenceType()) ||
      (RHSTy->isArrayType() &&
       RHSTy->getArrayElementTypeNoTypeQual()->isWebAssemblyReferenceType())) {
    return InvalidOperands(Loc, LHS, RHS);
  }
...
QualType Sema::CheckAssignmentOperands(Expr *LHSExpr, ExprResult &RHS,
  ...
  if (RHSType->isWebAssemblyTableType()) {
    Diag(Loc, diag::err_wasm_table_assignment);
    return QualType();
  }
...
QualType Sema::CheckAddressOfOperand(ExprResult &OrigOp, SourceLocation OpLoc) {
  ...
  if (Context.getTargetInfo().getTriple().isWasm()) {
    ...
    if (OpTy->isWebAssemblyTableType()) {
      Diag(OpLoc, diag::err_wasm_addrof_table)
          << OrigOp.get()->getSourceRange();
      return QualType();
    }
  }
