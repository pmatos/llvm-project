...
NamedDecl *Sema::ActOnVariableDeclarator(
  ...
  if (const Type *ElTy = NewVD->getType()->getArrayElementTypeNoTypeQual()) {
...
Sema::ActOnFunctionDeclarator(Scope *S, Declarator &D, DeclContext *DC,
  ...
  for (const ParmVarDecl *Param : NewFD->parameters()) {
    ...
    if (Context.getTargetInfo().getTriple().isWasm()) {
      if (PT->isWebAssemblyTableType()) {
        Diag(Param->getTypeSpecStartLoc(),
             diag::err_wasm_table_as_function_argument);
        D.setInvalidType();
      }
      }
  }
...
void Sema::AddInitializerToDecl(Decl *RealDecl, Expr *Init, bool DirectInit) {
  ...
  if (Init && !Init->getType().isNull() &&
    ...
    Diag(Init->getExprLoc(), diag::err_wasm_table_assignment);
    VDecl->setInvalidDecl();
    return;
  }
